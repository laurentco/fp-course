...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List b
  (<*>) Nil _ = Nil
  (<*>) (f1 :. funs) list = map f1 list ++ ((<*>) funs list)
...
-- lift2 (a -> f1) -> k a -> k1 ( b -> c) = liftOne f k 
  
liftOne :: Applicative k => (a -> b) -> k a -> k b
liftOne f k = pure f <*> k
...
lift3 ::
  ...
  -> k c
  -> k d
lift3 a2b2c2d ka kb kc = a2b2c2d <$> ka <*> kb <*> kc
