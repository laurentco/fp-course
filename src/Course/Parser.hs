...
(|||) ::
  ...
  -> Parser a
  -> Parser a
  -- parse p1 :: Input -> ParseResult a
  -- parse p2 :: Input -> ParseResult a
...
instance Monad Parser where
  (=<<) ::
    ...
    -- parse p input 
  (=<<) f p = P ( \input -> onResult (parse p input) (\i a -> parse (f a) i) )
...
satisfy ::
  ...
  -- unexpectedCharParser :: c -> Parser c
  -- character >>= \c -> bool (unexpectedCharParser c) (pure c) (p c)
satisfy p = 
  character >>= lift3 bool unexpectedCharParser pure p     -- lift3 essentially passes the c to each of the 3 functions (DI)
  -- character >>= if p then pure else unexpectedCharParser
  -- bool unexpectedCharParser
...
list1 =
  \p ->
  -- is anything on the left of <- used in the right of <- ? no => we can use liftN
    do x <- p
       y <- list p
       pure ( x :. y)
  -- lift2 (:.) p (list p)   same as lift2 (:.) <*> list
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = sequence -- foldRight (.:.) (pure Nil)
...
surnameParser =
  do 
     ...
     -- or lift3 because none of the left of the <- is on the right
...
smokerParser ::
  ...
  -- (Char -> Bool) -> (Parser Char -> Parser Bool)
